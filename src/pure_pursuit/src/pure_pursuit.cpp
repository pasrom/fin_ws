                             ...
                             lookahead_frame_id_("lookahead")
{
  ...
  // get joy
  sub_joy_ = nh_.subscribe<sensor_msgs::Joy>("joy", 10, &PurePursuit::joyCallback, this);
...
void PurePursuit::joyCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
    if (joy->buttons.at(4)){
        resetAndStop = true;
        goal_reached_ = true;
...
void PurePursuit::goalCallback(geometry_msgs::PoseStamped msg)
{
    tf::Quaternion q1;
    nav_msgs::Path plan;
    ...
    // int carx = c2d_pose(msg.pose.position.x);
    // int cary = c2d_pose(msg.pose.position.y);
    // tf::quaternionMsgToTF(msg.pose.orientation, q1);
    // int goalYaw = c2d_yaw(tf::getYaw(q1));
    // // final vehicle pose on the path
    // nav_msgs::Path plan;
    plan.header.stamp = ros::Time::now();

    plan.header.frame_id = msg.header.frame_id;
    // geometry_msgs::PoseStamped pose;
    // pose.header.frame_id = msg.goal.header.frame_id;
    // pose.header.seq = 1;
    // pose.pose.position.x = carx;
    // pose.pose.position.y = cary;
    // pose.pose.position.z = 0.0;
    // pose.pose.orientation = tf::createQuaternionMsgFromYaw(goalYaw);
    geometry_msgs::PoseStamped pose;
