...
class OdomPublisher:
  def __init__(self):
    ...
    self.orbSlamStatus_sub = rospy.Subscriber('orb/orb_slam_status', Int8, self.orbSlamStatus_sub_callback)    
    self.sub_joy = rospy.Subscriber('joy', Joy, self.sub_joy_callback)    
  ...
  def orbSlamStatus_sub_callback(self, msg):
    if msg.data == 1:
      self.pose['x'] = 0
      self.pose['y'] = 0
      self.pose['th'] = 0
